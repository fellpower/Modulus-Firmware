...
fn drawStatusCard(logical: *fb.LogicalFb, theme: tokens.Theme, state: *const State, r: geom.Rect) void {
    ...
    font.drawTextRole(logical, dot.x + 34, r.y + 16, "C6 Wireless Coprocessor", theme.on_surface, .title_m);
    font.drawTextRole(logical, dot.x + 34, r.y + 51, if (state.c6_connected) "ESP-Hosted / SDIO connected" else "C6 not connected", theme.on_surface_variant, .body_m);
...
pub fn paint(logical: *fb.LogicalFb, theme: tokens.Theme, state: *const State, enter_t: f32) Layout {
    ...
    tool_chrome.paintTitle(logical, theme, lay.header.back.x + lay.header.back.w + tokens.Space.sm, lay.header.back.y, "C6");
